...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
    position = 'right',
    colors = ['#18181b', '#27272a', '#3f3f46'], // Zinc shades
    items = [],
    socialItems = [],
    displaySocials = true,
                            ...
                            {items && items.length ? (
                                items.map((it, idx) => (
                                    <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                                        <Link
                                            className="sm-panel-item relative text-white font-semibold text-[3rem] md:text-[4rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]"
                                            href={it.link}
...
.sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; width: clamp(260px, 38vw, 420px); height: 100%; background: black; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; border-left: 1px solid rgba(255,255,255,0.1); }
@media (max-width: 768px) { .sm-scope .staggered-menu-panel { width: 100%; } }
...
@media (max-width: 768px) { .sm-scope .sm-prelayers { width: 100%; } }
